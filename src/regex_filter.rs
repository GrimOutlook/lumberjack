...
#[derive(Debug, Error)]
enum RegexFilterError {
    // TODO: Determine if this is needed. Currently not needed because using an empty filter just
    // returns the log split on newlines
    // #[error("Filter is not set")]
    // FilterNotSet,
    #[error("Filter does not apply to text: {0}")]
    FilterMismatch(String),
    #[error("Filter partial match to text: {0}")]
    FilterPartialMatch(String),
}
...
impl RegexFilter {
    ...
    pub fn parse(&self, text: &str) -> Result<Vec<Field>> {
        ...
        let Some(captures) = filter.captures(text) else {
            bail!(RegexFilterError::FilterMismatch(text.to_owned()));
        };
        ...
        for field_info in self.field_info.iter() {
            let Some(field_text) = captures.get(field_info.field_index) else {
                // NOTE: I will be beyond shocked if this code could actually be hit.
                debug!(
                    "Message '{}' is missing match group #{}",
                    text, field_info.field_index
                );
                bail!(RegexFilterError::FilterPartialMatch(text.to_owned()));
            };
